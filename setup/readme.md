...
* Fabric Manager (H100/A100 등 고사양 GPU용)
  ```
  sudo systemctl status nvidia-fabricmanager
  ```
  G6가 아닌 P4/P5 같은 고사양 인스턴스를 쓰신다면, GPU 간 NVLink 통신을 관리하는 Fabric Manager 가 필요.
  Fabric Manager는 여러 개의 GPU가 NVLink(GPU 간 초고속 고속도로)로 연결되어 있을 때, 이들이 하나의 거대한 GPU처럼 유기적으로 작동하도록 통신 경로를 관리.

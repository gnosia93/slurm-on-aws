## Scaling Efficiency ##
스케일링 효율은 추가된 GPU가 실제 훈련 속도 향상으로 얼마나 효과적으로 전환되는지를 나타내는 지표입니다. 이상적인 선형 스케일링에서는 GPU를 2배로 늘리면 처리량도 2배가 되지만, 실제로는 통신 오버헤드, 동기화 대기, 파이프라인 버블 등으로 인해 이 비율이 줄어들며, 클러스터 규모와 네트워크 토폴로지, 병렬화 전략에 따라 보통 60-90% 수준의 효율을 보입니다.
```
